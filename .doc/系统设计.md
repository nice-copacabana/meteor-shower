...
## 14. 高效提示词（Prompt）共享与复用
- 目标：沉淀并复用“优秀使用者”的高质量提示词与工作流，使其可在多工具间快速迁移与一键应用。
- 形态：
  - Prompt 清单：分门别类（重构/测试生成/审查/SOP/故障定位/RAG检索/安全审计）。
  - Few-shot 模板：可插槽变量（语言/框架/路径/规范），与项目规则联动。
  - 命令化封装：转为 Gemini `.gemini/commands/*.toml`、CLI 子命令或 UI 按钮。
- 元数据：id、名称、用途、适配工具、变量 schema、示例输入/输出、风险分级、评分与下载量。
- 应用流程：
  1) 选择 Prompt → 2) 填写变量 → 3) dry-run 预览 → 4) 一键应用至目标工具（或生成脚本）。
- 安全与治理：
  - 敏感信息占位与模板化；提示注入防护；输出限制与审计；云端评级与黑名单机制。
