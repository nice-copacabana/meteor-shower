...
export class ClaudeAdapter implements Adapter {
  async plan(ctx: ApplyContext): Promise<DiffResult> {
    ...
    // 基于上下文和模板变量计算将要创建的文件
    const changes = [
      { path: '~/.claude/claude_desktop_config.json', kind: 'create' }
    ];

    return {
      changes,
      summary: `将创建 ${changes.length} 个 Claude 配置文件`
    };
  }
  ...
  async apply(ctx: ApplyContext): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.green('✅ Claude 配置应用完成'));
    } catch (error) {
      console.error(chalk.red('❌ Claude 配置应用失败:'), error);
      throw error;
    }
  }
  ...
  async rollback(ctx: ApplyContext): Promise<void> {
    ...
    try {
      ...
      console.error(chalk.red('❌ Claude 配置回滚失败:'), error);
      throw error;
    }
  }
