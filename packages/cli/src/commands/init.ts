...
async function collectVariables(template: string, toolset: string[]) {
  ...
  const { projectName, persona, projectDescription } = await inquirer.prompt([
    ...
    },
    {
      ...
      message: '项目描述:',
      default: (answers: any) => `${answers.projectName} - AI 辅助开发项目`
    },
    {
      type: 'input',
      name: 'persona',
  ...
  if (toolset.includes('gemini')) {
    ...
    const gemini = await inquirer.prompt([
      ...
      },
      {
        ...
        default: 0.2,
        validate: (value) => {
          if (value < 0 || value > 2) return '请输入 0.0 到 2.0 之间的数字';
          return true;
        }
      },
      {
        ...
        default: 8192,
        validate: (value) => {
          if (value < 1 || value > 8192) return '请输入 1 到 8192 之间的数字';
          return true;
        }
      },
      {
  ...
  if (toolset.includes('claude')) {
    ...
    const claude = await inquirer.prompt([
      ...
      },
      {
        ...
        default: 4000,
        validate: (value) => {
          if (value < 1 || value > 4096) return '请输入 1 到 4096 之间的数字';
          return true;
        }
      }
    ]);
  ...
  if (toolset.includes('openai')) {
    ...
    const openai = await inquirer.prompt([
      ...
      },
      {
        ...
        default: 4096,
        validate: (value) => {
          if (value < 1 || value > 128000) return '请输入 1 到 128000 之间的数字';
          return true;
        }
      },
      {
 ...
 * 根据工具集获取推荐的模板选项
 */
function getTemplateChoices(toolset: string[]): Array<{ name: string; value: string }> {
  ...
  if (toolset.length === 1) {
    if (toolset.includes('gemini')) {
      choices.push(
        { name: 'Gemini 基础模板 (推荐)', value: 'gemini-basic' },
        { name: 'Gemini 高级模板', value: 'gemini-advanced' }
      );
    }
    if (toolset.includes('claude')) {
      choices.push(
        { name: 'Claude 基础模板 (推荐)', value: 'claude-basic' },
        { name: 'Claude 高级模板', value: 'claude-advanced' }
      );
    }
    if (toolset.includes('cursor')) {
      choices.push(
        { name: 'Cursor 基础模板 (推荐)', value: 'cursor-basic' },
        { name: 'Cursor Pro 模板', value: 'cursor-pro' }
      );
    }
    if (toolset.includes('openai')) {
      choices.push(
        { name: 'OpenAI 基础模板 (推荐)', value: 'openai-basic' },
        { name: 'OpenAI 高级模板', value: 'openai-advanced' }
      );
    }
  } else {
    // 多工具组合模板
    choices.push(
      { name: '多工具基础模板 (推荐)', value: 'multi-basic' },
      { name: '多工具高级模板', value: 'advanced-multi' },
      { name: '团队协作模板', value: 'team-collaboration' }
    );
  }
